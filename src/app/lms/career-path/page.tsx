...
import { LMS_CARD_CLASS, LMS_CARD_TITLE, LMS_PRIMARY_LINK_CLASS, LMS_SECTION_TITLE } from '../constants';
...
export default function LmsCareerPathPage() {
  ...
  return (
    <div className="space-y-8 pb-10">
      <LmsPageHeader
        title="Career path"
        subtitle="Mission system with adaptive roadmap. Quizzes, notes, events, and resume work feed this view natively."
        actions={
          <div className="flex items-center gap-3">
            {hasStarted ? (
              <button
                type="button"
                className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl px-4 py-2 text-[0.8125rem] font-medium text-rose-600 transition-colors hover:bg-rose-50"
                onClick={handleReset}
              >
            ...
            <button
              type="button"
              className={`${LMS_PRIMARY_LINK_CLASS} disabled:cursor-not-allowed disabled:opacity-50 disabled:scale-100 disabled:hover:scale-100`}
              disabled={hasStarted}
              onClick={() => {
                careerStart();
      ...
      <section className="space-y-4 rounded-2xl border border-violet-100 bg-white/70 p-5 shadow-sm transition-shadow duration-200 hover:shadow-md sm:p-6">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <h2 className={`${LMS_CARD_TITLE} text-gray-900`}>{careerMission.headline}</h2>
        </div>
          ...
          <div className={`${LMS_CARD_CLASS} transition-all duration-200 hover:shadow-md lg:col-span-2`}>
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className={`${LMS_CARD_TITLE} text-gray-900`}>{dynamicRole}</p>
