...
export function CourseLessonsClient({ courseId, modules, mode }: { courseId: string; modules: LmsCourseModule[]; mode: Mode }) {
  ...
  const flat = useMemo(() => flattenCourseLessons(modules), [modules]);
  ...
  const openLesson = (idx: number) => {
    ...
    toast.push({ title: 'Lesson opened', message: lesson?.lessonTitle ?? 'Lesson', tone: 'info' });
    if (lesson) {
      router.push(`/lms/courses/${courseId}/lessons/${lesson.lessonId}${mode === 'review' ? '?mode=review' : ''}`);
    }
  };
