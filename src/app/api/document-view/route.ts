...
function getBackendDocumentViewUrl(search: string): string {
  const apiBase = (process.env.BACKEND_INTERNAL_URL || defaultBackendBase).replace(/\/+$/, '');
  return `${apiBase}/document-view${search}`;
}

/**
 * Same-origin proxy so profile documents open inline in a new tab (S3 CORS-safe).
 */
export async function GET(req: NextRequest) {
  ...
  const target = getBackendDocumentViewUrl(search);
  ...
  try {
    ...
    console.error('[api/document-view] backend proxy failed', {
      target,
