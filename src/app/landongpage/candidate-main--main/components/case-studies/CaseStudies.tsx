...
import type { CandmainCaseStudy } from '@/lib/candmain-landing'
...
function CaseStudyCard({
  ...
  study: CandmainCaseStudy
  index: number
  labels: { problem: string; checks: string; helps: string; showLess: string; outcome: string }
}) {
  ...
  return (
    ...
    >
      <div className="h-1 w-full" style={{ background: `linear-gradient(90deg, ${study.color}, transparent)` }} />

      <div className="p-8">
        <div className="flex items-center justify-between mb-6">
          <span
            className="tag-pill text-xs font-semibold"
            style={{ background: `${study.color}10`, color: study.color, border: `1px solid ${study.color}20` }}
          >
            {study.tag}
          </span>
          <span className="text-5xl font-black text-[rgba(15,23,42,0.04)]">0{index + 1}</span>
        </div>

        <h3 className="text-xl font-bold text-text-primary leading-tight mb-6">{study.title}</h3>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          {study.results.map((result, resultIndex) => {
            ...
            return (
              ...
              >
                <div className="text-xl font-bold tracking-tight mb-1" style={{ color }}>{result.value}</div>
        ...
        >
          <div className="space-y-4 pb-6">
            {[
              { label: labels.problem, content: study.challenge },
              { label: labels.checks, content: study.research },
              { label: labels.helps, content: study.strategy },
            ].map((section) => (
              <div key={section.label}>
                <div className="text-xs font-bold text-text-muted uppercase tracking-wider mb-2">{section.label}</div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-[rgba(15,23,42,0.06)]">
          ...
          >
            {expanded ? labels.showLess : labels.helps}
          </button>
