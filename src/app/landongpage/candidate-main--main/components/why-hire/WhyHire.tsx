    ...
    <section className="section bg-[#0F172A] relative overflow-hidden" id="why-hire">
      ...
      <div className="container relative z-10">
        ...
        >
          ...
          >
            <span className="text-xs font-bold text-blue-400 uppercase tracking-wider">{w.tag}</span>
          </div>
          <h2 className="text-display-xl text-white mb-4">
            {w.title}{' '}
            <span className="gradient-text-blue">{w.titleAccent}</span>
          </h2>
          <p className="text-white/50 text-lg max-w-2xl mx-auto">{w.subtitle}</p>
        </motion.div>

        <div className="max-w-3xl mx-auto">
          <div className="grid grid-cols-[1fr_1fr] gap-4 mb-4">
            <div className="text-center py-3 px-4 rounded-xl bg-white/5 border border-white/10">
              <span className="text-xs font-bold text-white/40 uppercase tracking-wider">{w.othersHeader}</span>
            </div>
            <div
              className="text-center py-3 px-4 rounded-xl"
              style={{ background: 'rgba(40,168,225,0.2)', border: '1px solid rgba(40,168,225,0.35)' }}
            >
              <span className="text-xs font-bold text-blue-400 uppercase tracking-wider">{w.portalHeader}</span>
            </div>
          </div>

          <div className="space-y-3">
            {w.comparisons.map((row, i) => (
              <motion.div
                key={i}
              ...
              >
                ...
                <div
                  ...
                  style={{ background: 'rgba(40,168,225,0.12)', border: '1px solid rgba(40,168,225,0.3)' }}
                >
                  <div
          ...
          >
            <p className="text-white/80 text-base font-medium leading-relaxed">
              {w.insight}
              <br />
              <span className="text-white font-bold">{w.insightBold}</span>
            </p>
          </motion.div>
        </div>
      </div>
