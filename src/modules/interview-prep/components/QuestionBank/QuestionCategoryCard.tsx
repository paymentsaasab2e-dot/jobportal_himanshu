  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="rounded-full bg-[#2098C8]/10 px-2.5 py-0.5 text-xs font-bold text-[#2098C8]">
          {category.count}
        </span>
      </div>
      <p className="mt-2 text-sm font-normal text-gray-500 leading-relaxed">{category.description}</p>
      <span className="mt-4 inline-block text-sm font-bold text-[#2098C8] opacity-0 transition-opacity duration-200 group-hover:opacity-100">
        Open bank →
      </span>
    </button>
  );
