'use client';

import { Mic2 } from 'lucide-react';
import { TokenCoinIcon } from '@/components/tokens/TokenCoinIcon';

type MockStartButtonProps = {
  onStart: () => void;
};

export function MockStartButton({ onStart }: MockStartButtonProps) {
  return (
    <button
      type="button"
      onClick={onStart}
      className="btn-brand inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-2xl px-8 py-3.5 text-sm font-bold"
    >
      <Mic2 className="h-5 w-5" strokeWidth={2} aria-hidden />
      Start mock · <TokenCoinIcon className="h-5 w-5" /> 25
    </button>
  );
}
