              ...
              {request.questions.map((q, i) => (
                <div key={`${request.id}-aq-${i}`}>
                  ...
                  <WritingAssistField
                    ...
                    className="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-[#0A66C2]"
                  />
                </div>
              ))}
              <button
                ...
                className="inline-flex h-11 w-full items-center justify-center gap-1.5 rounded-full bg-[#0A66C2] text-sm font-semibold text-white disabled:opacity-50"
              >
                <Check className="h-4 w-4" />
                Send response
              </button>
